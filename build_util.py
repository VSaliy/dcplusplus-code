...
class Dev:
    ...
    def prepare(self):
        if not self.env['verbose']:
            ...
            self.env['GCHCOMSTR'] = (
                "Compiling $TARGET (static precompiled header)"
            )
            self.env['GCHSHCOMSTR'] = (
                "Compiling $TARGET (shared precompiled header)"
            )
        ...
        if self.env['msvcproj']:
            if 'msvs' not in self.env['TOOLS']:
                raise Exception(
                    'This is not an MSVC build; specify tools=default'
                )
    ...
    def finalize(self):
        if self.env['msvcproj']:
            path = (
                self.msvcproj_path + 'DCPlusPlus' +
                self.env['MSVSSOLUTIONSUFFIX']
            )
    ...
    def prepare_build(
        self, source_path, name, source_glob='*.cpp', in_bin=True,
        precompiled_header=None, recursive=False
    ):
    ...
    def build_lib(
        self, env, target, sources, msvcproj_glob=None, msvcproj_name=None,
        shared=False
    ):
        if env['msvcproj']:
            ...
            path = (
                self.msvcproj_path + msvcproj_name + env['MSVSPROJECTSUFFIX']
            )
            env.Precious(path)
        ...
        return (
            env.SharedLibrary(target, sources) if shared
    ...
    def i18n(self, source_path, buildenv, sources, name):
        ...
        for po_file in p_oze:
            ...
            buildenv.MoBuild(
                locale_path + 'LC_MESSAGES/' + name + '.mo', po_file,
...
def CheckPKGConfig(context, version):
    ...
    ret = context.TryAction(
        'pkg-config --atleast-pkgconfig-version=%s' % version
    )[0]
    context.Result(ret)
    return ret
