...
void MainWindow::on(WindowManagerListener::Window, const string& id) throw() {
	if(false) { }
#define compare_id(frame) else if(typeid(frame).name() == id) callAsync(std::tr1::bind(&frame::openWindow, getTabView()))
	...
	///@todo handle PMs & file lists

#undef compare_id
}

void MainWindow::on(WindowManagerListener::Hub, const string& /*name*/, const string& address) throw() {
